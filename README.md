...
- Durable history contains only the normalized safe event fields shown by the UI—never prompts, private reasoning, raw tool inputs/outputs, credentials, URLs, or raw payloads. The local write surface is limited to authenticated same-origin JSON settings updates under `/data`; Gateway operations remain strictly read-only.
